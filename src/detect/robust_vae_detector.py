...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Robust VAE detector')
    parser.add_argument('cmd', choices=['compare', 'histogram'], 
...
def VAEdetector(target, Xtrain, Xorg, Xadv, Xrobust, ax, args):
    if not args.robust: 
    ...
    rl = vae.get_losses(Xrobust).numpy()
    ...
    rl = np.random.choice(rl, bl.shape[0])
    ...
    ax.plot(recall, precision, marker=markers[target], markersize=10, label=f"EM+VAE", lw=1)
    ...
    print("> Robust rate:", (rl < threshold).sum()/rl.shape[0])
    print("> Advesarial rate:", (al < threshold).sum()/al.shape[0])
    
    data = np.concatenate([bl, al, rl], axis=0)
    lab  = np.concatenate([np.zeros(bl.shape),  2*np.ones(rl.shape), np.ones(al.shape)], axis=0)
    dataset = pd.DataFrame({'label': lab, 'losses': data}, columns=['label', 'losses'])

    fig, ax = plt.subplots(figsize=(12, 10))
    sns.histplot(data=dataset, x='losses', hue='label', bins=50, palette=["C0", "C3", "C1"])
    ...
    plt.legend(['Threshold', 'CW', 'FGSM', 'Benign' ], fontsize=30)
    image_path = f"imgs/robust-{args.dataset}-{args.attack_method}/"
    if not os.path.exists(image_path):
        os.makedirs(image_path)
    
    print(plt.gca().get_legend_handles_labels())
    plt.tight_layout()
    plt.savefig(f"{image_path}{args.target}.pdf")
    plt.close()
    return auc_score
...
def pca(XtrainA, XorgA):
    ...
    index = np.concatenate([np.arange(0,100), np.arange(110,130), np.arange(170,180)], dtype=np.int32)
    ...
    ax.scatter(Xadv[:, 0], Xadv[:, 1], Xadv[:, 2], c=colors[2], edgecolor="k", label="Advesarial: Class 2->1", marker='^', s=80)
    # ax.w_xaxis.set_ticklabels([])
    # ax.w_yaxis.set_ticklabels([])
    # ax.w_zaxis.set_ticklabels([])
    ax.set_title("Embedding of adversarial samples", fontsize=20)
    leg = plt.legend(title='Samples Embeddings', title_fontsize=20, loc='upper right', fontsize=20)
...
def load_data(target, VICTIM, ATTACK, args):
    ...
    Xorg = np.load(f"data/robust-{args.dataset}-{VICTIM}-{ATTACK}{target}/Xorg.npy")[:, :60]
    Xadv = np.load(f"data/robust-{args.dataset}-{VICTIM}-{ATTACK}{target}/Xadv.npy")[:, :60]
    Xrobust = np.load(f"data/robust-{args.dataset}-{VICTIM}-{ATTACK}{target}/Xrobust.npy")[:, :60]
    y_test = np.load(f"data/robust-{args.dataset}-{VICTIM}-{ATTACK}{target}/yorg.npy")
    y_adv  = np.load(f"data/robust-{args.dataset}-{VICTIM}-{ATTACK}{target}/yadv.npy")
    y_robust = np.load(f"data/robust-{args.dataset}-{VICTIM}-{ATTACK}{target}/yrobust.npy")

    y_test = np.squeeze(y_test)
    Xtrain, Xtest = train_test_split(Xorg[y_test==target], test_size=0.2, random_state=42)
    Xrobust = Xrobust[y_robust==target]
    scaler = preprocessing.StandardScaler().fit(Xtrain)
    normalizer = preprocessing.Normalizer().fit(Xtrain)
    ...
    Xadv_scaled = scaler.transform(Xadv)
    Xadv_normalized = normalizer.transform(Xadv)
    Xrobust_normalized = normalizer.transform(Xrobust)

    Xtrain = Xtrain_normalized
    Xorg = Xorg_normalized
    Xadv = Xadv_normalized
    Xrobust = Xrobust_normalized

    return Xtrain, Xorg, Xadv, Xrobust
...
def draw_compare(args):
    ...
    Xtrain, Xorg, Xadv = load_data(target, VICTIM, ATTACK)
...
def draw_histogram(args):
    ...
    ATTACK = f"{METHOD}{target}"
    Xtrain, Xorg, Xadv, Xrobust = load_data(target, VICTIM, METHOD, args)
    score = VAEdetector(target, Xtrain, Xorg, Xadv, Xrobust, ax, args)
    ...
    scores.append(score)
    fig.tight_layout()
    plt.savefig(f"imgs/robust-{args.dataset}-{METHOD}{target}.png")
    plt.close()
    print(scores)
