...
from sklearn.metrics import precision_recall_curve, top_k_accuracy_score
import time 
import matplotlib
...
def parse_args():
    ...
    parser.add_argument('cmd', choices=['compare', 'histogram', 'class'], 
...
def VAEdetector(target, Xtrain, Xorg, Xadv, ax, args):
    ...
    ax.plot(recall, precision, marker=markers[target], markersize=10, label=f"{target}", lw=3, c=colors[target])
    ...
    sns.histplot(data=dataset, x='losses', hue='label', bins=50)
    ...
    plt.legend(['Threshold', 'Adversarial Samples', 'Benign Samples'], fontsize=30, loc='best')
...
def draw_classes(args):
    ...
    ax.grid()
    ax.set_title("Precision-Recall Graph", fontsize=28)
    ax.set_xlim(0, 1.0)
    ax.set_ylim(0.5, 1.0)
    scores.append(score)
    fig.tight_layout()
    plt.savefig("imgs/targetedPGD/cifar-prcurve.pdf")
    print(scores)
...
def draw_histogram(args):
    ...
    ATTACK = f"{METHOD}{target}"
    Xtrain, Xorg, Xadv = load_data(target, VICTIM, METHOD, args)
    score = VAEdetector(target, Xtrain, Xorg, Xadv, ax, args)
    scores.append(score)
    ...
    ax.set_ylim(0.5, 1.0)
    fig.tight_layout()
    plt.savefig(f"imgs/{args.dataset}-{METHOD}.pdf")
    plt.close()
    print(scores)
